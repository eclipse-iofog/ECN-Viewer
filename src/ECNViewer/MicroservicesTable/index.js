...
export default function MicroservicesTable({
  ...
  const microservices = application.microservices?.filter(
    (m) =>
      m.name.toLowerCase().includes(filter) ||
      m.status.status.toLowerCase().includes(filter) ||
      (selectAgent &&
        (reducedAgents.byUUID[m.iofogUuid] || { name: "" }).name
          .toLowerCase()
          .includes(filter)),
  );
          ...
          return (
            ...
            >
              ...
              <TableCell style={{ width: "200px" }}>
                <span>
                  {row.status?.status}
                  {row.status?.status === "PULLING" &&
                    ` (${row.status?.percentage !== undefined ? row.status?.percentage.toFixed(2) : 0}%)`}
                </span>
                {row.status?.errorMessage && (
                  <>
                    <br />
                    <span>{row.status?.errorMessage}</span>
                  </>
                )}
              </TableCell>
              {selectAgent && (
              ...
              <TableCell style={{ width: "200px" }}>
                {row.ports.map((p) => (
                  <div key={p.internal} style={{ paddingBottom: "5px" }}>
                    {p.internal}:{p.external}/
                    {p.protocol === "udp" ? "udp" : "tcp"}
                  </div>
                ))}
              </TableCell>
              {showVolumes && (
                <TableCell style={{ width: "200px" }}>
                  {row.volumeMappings.map((p) => (
                    <div key={p.id} style={{ paddingBottom: "5px" }}>
                      {p.hostDestination}:{p.containerDestination}:
                      {p.accessMode}
                    </div>
                  ))}
                </TableCell>
